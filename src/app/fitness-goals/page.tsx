...
export default function FitnessGoalsPage() {
    ...
    return (
        <div className="relative flex flex-col min-h-screen bg-cover bg-center text-white" style={{backgroundImage: "url('https://i.pinimg.com/1200x/e2/1f/ef/e21fef81dcf0b7e4acbf5609b11997b7.jpg')"}}>
            <div className="absolute inset-0 bg-black/50 z-0"/>
            <div className="relative z-10 flex flex-col min-h-screen">
                <header className="container mx-auto px-4 py-6 z-10">
                    <div className="flex justify-between items-center"><GlowHerLogo /><Button variant="ghost" onClick={() => router.push('/')} className="text-white hover:bg-white/10 hover:text-white"><ChevronLeft className="mr-2 h-4 w-4" />Back to Dashboard</Button></div>
                </header>

                <main className="flex-grow container mx-auto px-4 py-8 space-y-8">
                    <div className="text-center">
                        <h1 className="font-headline text-4xl md:text-5xl font-bold">Fitness Goal Planner</h1>
                        <p className="mt-2 text-lg text-white/80">Move mindfully through your health journey.</p>
                    </div>

                    <Card className="max-w-md mx-auto shadow-lg bg-white/20 backdrop-blur-sm border-white/30 text-white"><CardHeader><CardTitle>Let's Personalize Your Plan</CardTitle></CardHeader><CardContent><div className="flex items-center justify-between space-x-2"><Label htmlFor="pregnancy-mode" className="text-base">Are you currently pregnant?</Label><Switch id="pregnancy-mode" checked={isPregnant} onCheckedChange={handlePregnancyToggle} /></div></CardContent></Card>

                    {isPregnant ? (
                        // --- PREGNANCY MODE UI ---
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            <div className="space-y-8">
                                <Card className="shadow-lg bg-white/20 backdrop-blur-sm border-white/30 text-white">
                                    <CardHeader><div className="flex justify-between items-center"><CardTitle className="flex items-center gap-2"><Target /> Your Movement Goals</CardTitle><Button variant="ghost" size="icon" onClick={() => setIsEditingGoals(!isEditingGoals)}><Edit className="h-4 w-4"/></Button></div><CardDescription className="text-white/80">Set your gentle movement targets for the week.</CardDescription></CardHeader>
                                    <CardContent>
                                        {isEditingGoals ? (
                                            <Form {...pregnancyGoalForm}><form onSubmit={pregnancyGoalForm.handleSubmit(onPregnancyGoalSubmit)} className="space-y-4">
                                                <FormField control={pregnancyGoalForm.control} name="days" render={({ field }) => (<FormItem><FormLabel>Days to Move This Week</FormLabel><FormControl><Input type="number" {...field} className="bg-white/10 border-white/20 text-white placeholder:text-slate-400"/></FormControl><FormMessage/></FormItem>)}/>
                                                ...
                                                <FormField control={pregnancyGoalForm.control} name="trackMood" render={({ field }) => (<FormItem className="flex flex-row items-center justify-between rounded-lg border border-white/30 p-3 shadow-sm"><FormLabel>Track Energy/Mood?</FormLabel><FormControl><Switch checked={field.value} onCheckedChange={field.onChange} /></FormControl></FormItem>)} />
                                                <Button type="submit" className="w-full bg-primary hover:bg-primary/90 text-primary-foreground">Save Goals</Button>
                                            </form></Form>
                                        ) : (
                                            <div className="space-y-4">
                                                <div className="flex justify-between p-4 rounded-lg bg-white/10"><p className="font-medium">Weekly Goal</p><p className="font-bold">{goalDays} days</p></div>
                                                <div className="flex justify-between p-4 rounded-lg bg-white/10"><p className="font-medium">Preferred Activity</p><p className="font-bold">{pregnancyGoalForm.getValues('activityType')}</p></div>
                                            </div>
                                        )}
                                    </CardContent>
                                </Card>
                                        ...
                                        <Form {...pregnancyLogForm}><form onSubmit={pregnancyLogForm.handleSubmit(onPregnancyLogSubmit)} className="space-y-4">
                                            ...
                                            <div className="grid grid-cols-2 gap-4">
                                                <FormItem>
                                                    ...
                                                    <Select onValueChange={(value) => setDuration(d => ({ ...d, hours: Number(value) }))} value={String(duration.hours)}><FormControl><SelectTrigger><SelectValue/></SelectTrigger></FormControl><SelectContent>
                                                            {[...Array(5).keys()].map(h => <SelectItem key={h} value={String(h)}>{h} hr</SelectItem>)}
                                                        </SelectContent></Select>
                                                </FormItem>
                                                <FormItem>
                                                    <FormLabel>Minutes</FormLabel>
                                                    <Select onValueChange={(value) => setDuration(d => ({ ...d, minutes: Number(value) }))} value={String(duration.minutes)}><FormControl><SelectTrigger><SelectValue/></SelectTrigger></FormControl><SelectContent>
                                            ...
                                            <FormField control={pregnancyLogForm.control} name="notes" render={({ field }) => (<FormItem><FormLabel>Notes (optional)</FormLabel><FormControl><Textarea placeholder="Any thoughts on today's movement?" {...field} className="bg-white/10 border-white/20 placeholder:text-slate-400" /></FormControl><FormMessage/></FormItem>)}/>
                                            <Button type="submit" className="w-full bg-primary hover:bg-primary/90 text-primary-foreground">Log Movement</Button>
                                        </form></Form>
                                    </CardContent>
                                </Card>
                                 <Card className="shadow-lg bg-white/20 backdrop-blur-sm border-white/30 text-white">
                                    <CardHeader>
                                        ...
                                        <CardDescription className="text-white/80">Your logged minutes over the last 7 days.</CardDescription>
                                    </CardHeader>
                                    <CardContent>
                            ...
                            <div className="space-y-8">
                               <Card className="shadow-lg bg-white/20 backdrop-blur-sm border-white/30 text-white">
                                    <CardHeader><CardTitle className="flex items-center gap-2"><BarChart/> Weekly Goal Progress</CardTitle><CardDescription className="text-white/80">You've moved on {completedDays} of your {goalDays} day goal.</CardDescription></CardHeader>
                                    <CardContent className="flex justify-center items-center h-48">
                                        <ResponsiveContainer width="100%" height="100%">
                                            <PieChart>
                                                <Pie data={[{ value: 1 }]} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius="70%" outerRadius="90%" fill="hsla(0,0%,100%,0.1)" startAngle={90} endAngle={-270} paddingAngle={0} cornerRadius={50} />
                                                ...
                                                <text x="50%" y="50%" textAnchor="middle" dominantBaseline="middle" className="text-4xl font-bold fill-white">{`${progressPercentage}%`}</text>
                                                <text x="50%" y="65%" textAnchor="middle" dominantBaseline="middle" className="text-sm fill-white/80">completed</text>
                                            </PieChart>
                                        </ResponsiveContainer>
                                    </CardContent>
                                </Card>
                                <Card className="shadow-lg bg-white/20 backdrop-blur-sm border-white/30 text-white">
                                    ...
                                    <CardContent className="space-y-2">
                                        <div className={cn("flex items-center gap-4 p-3 rounded-lg", streak > 0 ? "bg-amber-500/20" : "bg-white/10")}>
                                            <Award className={cn("h-6 w-6", streak > 0 ? "text-amber-400 fill-amber-400" : "text-white/50")} />
                                            <div>
                                                <p className="font-semibold">Consistency Streak</p>
                                                <p className="text-sm text-white/80">{streak > 0 ? `You're on a ${streak}-day streak!` : "Log a workout today to start a streak."}</p>
                                            </div>
                                        </div>
                                        <div className={cn("flex items-center gap-4 p-3 rounded-lg", completedDays >= goalDays ? "bg-primary/20" : "bg-white/10")}>
                                            <Award className={cn("h-6 w-6", completedDays >= goalDays ? "text-primary fill-primary" : "text-white/50")} />
                                            <div>
                                                <p className="font-semibold">Weekly Goal</p>
                                                <p className="text-sm text-white/80">{completedDays >= goalDays ? "You hit your goal this week!" : "Keep going to reach your weekly goal."}</p>
                                            </div>
                                        </div>
                                    </CardContent>
                                </Card>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            <div className="space-y-8">
                                <Card className="shadow-lg bg-white/20 backdrop-blur-sm border-white/30 text-white">
                                    <CardHeader><div className="flex justify-between items-center"><CardTitle className="flex items-center gap-2"><Target /> Your Fitness Goals</CardTitle><Button variant="ghost" size="icon" onClick={() => setIsEditingGoals(!isEditingGoals)}><Edit className="h-4 w-4"/></Button></div><CardDescription className="text-white/80">Set and edit your daily and weekly targets.</CardDescription></CardHeader>
                                    <CardContent>
                                        {isEditingGoals ? (
                                            <Form {...defaultGoalForm}><form onSubmit={defaultGoalForm.handleSubmit(onGoalSubmit)} className="space-y-4">
                                                <FormField control={defaultGoalForm.control} name="steps" render={({ field }) => (<FormItem><FormLabel>Daily Steps Goal</FormLabel><FormControl><Input type="number" {...field} className="bg-white/10 border-white/20 placeholder:text-slate-400"/></FormControl><FormMessage/></FormItem>)}/>
                                                <FormField control={defaultGoalForm.control} name="workouts" render={({ field }) => (<FormItem><FormLabel>Weekly Workouts Goal</FormLabel><FormControl><Input type="number" {...field} className="bg-white/10 border-white/20 placeholder:text-slate-400"/></FormControl><FormMessage/></FormItem>)}/>
                                                <Button type="submit" className="w-full bg-primary hover:bg-primary/90 text-primary-foreground">Save Goals</Button>
                                            </form></Form>
                                        ) : (
                                            <div className="space-y-4"><div className="flex justify-between p-4 rounded-lg bg-white/10"><p className="font-medium">Daily Steps</p><p className="font-bold">{defaultGoalForm.getValues('steps').toLocaleString()}</p></div><div className="flex justify-between p-4 rounded-lg bg-white/10"><p className="font-medium">Workouts Per Week</p><p className="font-bold">{defaultGoalForm.getValues('workouts')}</p></div></div>
                                        )}
                                    </CardContent>
                                </Card>
                                 <Card className="shadow-lg bg-white/20 backdrop-blur-sm border-white/30 text-white">
                                    <CardHeader><CardTitle className="flex items-center gap-2"><Dumbbell/> Log Today's Activity</CardTitle><CardDescription className="text-white/80">Record your movement for today.</CardDescription></CardHeader>
                                    <CardContent>
                                        <Form {...defaultLogForm}><form onSubmit={defaultLogForm.handleSubmit(onLogSubmit)} className="space-y-4">
                                            <FormField control={defaultLogForm.control} name="steps" render={({ field }) => (<FormItem><FormLabel className="flex items-center gap-2"><Footprints/> Steps Taken</FormLabel><FormControl><Input type="number" {...field} className="bg-white/10 border-white/20 placeholder:text-slate-400"/></FormControl><FormMessage/></FormItem>)}/>
                                             ...
                                             <Button type="submit" className="w-full bg-primary hover:bg-primary/90 text-primary-foreground">Log Activity</Button>
                                        </form></Form>
                                    </CardContent>
                                </Card>
                            </div>
                            <div className="space-y-8">
                                 <Card className="shadow-lg bg-white/20 backdrop-blur-sm border-white/30 text-white">
                                    <CardHeader><CardTitle className="flex items-center gap-2"><BarChart/> Weekly Progress</CardTitle><CardDescription className="text-white/80">Your step count over the last 7 days.</CardDescription></CardHeader>
                                    <CardContent>
                                        ...
                                        <div className="mt-4"><Progress value={(defaultLogForm.getValues('steps') / defaultGoalForm.getValues('steps')) * 100} className="h-2 bg-white/20 [&>span]:bg-primary" /><p className="text-sm text-center mt-2 text-white/80">Today's progress: {defaultLogForm.getValues('steps').toLocaleString()} / {defaultGoalForm.getValues('steps').toLocaleString()} steps</p></div>
                                    </CardContent>
                                </Card>
                                {relevantSuggestions && !isPregnant ? (
                                    <Card className={cn("shadow-lg border", "bg-white/20 border-white/30")}>
                                        <CardHeader><CardTitle className="flex items-center gap-2"><relevantSuggestions.icon/> {relevantSuggestions.title}</CardTitle><CardDescription className="text-white/80">Exercises aligned with your current menstrual phase.</CardDescription></CardHeader>
                                    ...
                                    <Alert className="bg-white/20 border-white/30 text-white"><Info className="h-4 w-4" /><AlertTitle>Enter Your Cycle Data</AlertTitle><AlertDescription>To get personalized fitness suggestions, please enter your details in the Period Tracker.</AlertDescription></Alert>
                                )}
                            </div>
                        </div>
                    )}
                    ...
                    {isPregnant && (
                         <div className="mt-12 space-y-8">
                            ...
                            {pregnancyVideoUrl && (<Card className="shadow-xl bg-white/20 backdrop-blur-sm border-white/30"><CardHeader><CardTitle>Guided Workout for your {pregnancyTrimester}</CardTitle></CardHeader><CardContent><div className="aspect-video"><iframe className="w-full h-full rounded-lg" src={pregnancyVideoUrl} title={`Pregnancy Workout for ${pregnancyTrimester}`} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen></iframe></div></CardContent></Card>)}
                            <Card className="shadow-xl bg-white/20 backdrop-blur-sm border-white/30">
                                <CardHeader><CardTitle className="flex items-center gap-2"><HeartPulse /> Guided Prenatal Yoga</CardTitle><CardDescription className="text-white/80">A gentle yoga session suitable for all trimesters.</CardDescription></CardHeader>
