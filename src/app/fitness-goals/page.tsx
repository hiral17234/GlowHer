...
export default function FitnessGoalsPage() {
    ...
    return (
        <div className="relative flex flex-col min-h-screen bg-cover bg-center" style={{backgroundImage: "url('https://i.pinimg.com/1200x/e2/1f/ef/e21fef81dcf0b7e4acbf5609b11997b7.jpg')"}}>
            <div className="relative z-10 flex flex-col min-h-screen">
                ...
                <main className="flex-grow container mx-auto px-4 py-8 space-y-8">
                    <div className="text-center bg-black/20 backdrop-blur-sm p-4 rounded-lg">
                        <h1 className="font-headline text-4xl md:text-5xl font-bold text-white">Fitness Goal Planner</h1>
                        <p className="mt-2 text-lg text-white/80">Move mindfully through your health journey.</p>
                    </div>

                    <Card className="max-w-md mx-auto shadow-lg bg-white/20 backdrop-blur-sm border-white/30 text-white"><CardHeader><CardTitle className="text-white">Let's Personalize Your Plan</CardTitle></CardHeader><CardContent><div className="flex items-center justify-between space-x-2"><Label htmlFor="pregnancy-mode" className="text-base text-white">Are you currently pregnant?</Label><Switch id="pregnancy-mode" checked={isPregnant} onCheckedChange={handlePregnancyToggle} /></div></CardContent></Card>

                    {isPregnant ? (
                        // --- PREGNANCY MODE UI ---
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            <div className="space-y-8">
                                <Card className="shadow-lg bg-white/20 backdrop-blur-sm border-white/30 text-foreground">
                            ...
                            <div className="space-y-8">
                                 <Card className="shadow-lg bg-white/20 backdrop-blur-sm border-white/30 text-foreground">
                                    ...
                                    <CardContent>
                                        ...
                                        <div className="mt-4"><Progress value={(defaultLogForm.getValues('steps') / defaultGoalForm.getValues('steps')) * 100} className="h-2 bg-muted [&>span]:bg-primary" /><p className="text-sm text-center mt-2 text-muted-foreground">Today's progress: {defaultLogForm.getValues('steps').toLocaleString()} / {defaultGoalForm.getValues('steps').toLocaleString()} steps</p></div>
                                    </CardContent>
                                </Card>
                                {relevantSuggestions && !isPregnant ? (
                                    <Card className={cn("shadow-lg border", "bg-white/20 backdrop-blur-sm border-white/30 text-foreground")}>
                                        <CardHeader><CardTitle className="flex items-center gap-2"><relevantSuggestions.icon/> {relevantSuggestions.title}</CardTitle><CardDescription>Exercises aligned with your current menstrual phase.</CardDescription></CardHeader>
                                    ...
                                    <Alert className="bg-white/20 backdrop-blur-sm border-white/30 text-foreground"><Info className="h-4 w-4" /><AlertTitle>Enter Your Cycle Data</AlertTitle><AlertDescription>To get personalized fitness suggestions, please enter your details in the Period Tracker.</AlertDescription></Alert>
                                )}
                            </div>
                        </div>
                    )}
                    ...
                    {isPregnant && (
                         <div className="mt-12 space-y-8">
                            <div className="flex items-center justify-center mb-4 p-4 rounded-md bg-destructive/10 border border-destructive/20 backdrop-blur-sm">
